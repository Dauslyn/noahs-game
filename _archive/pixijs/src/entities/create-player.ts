...
/** Display scale multiplier for Ansimuz 48px sprites. */
const SPRITE_SCALE = 1.5;

/** Display scale for Noah PixelLab sprite. */
const NOAH_SCALE = 2.0;

/** Noah actual canvas size (PixelLab generates 56x56 for 48px side-view). */
const NOAH_SIZE = 56;
...
export function createPlayerEntity(
  ...
  // -- ECS components (scale set per-sprite below) --
  ...
  // -- Sprite: prefer Noah (PixelLab) over Ansimuz fallback --
  if (hasTexture('noah-east')) {
    const noahSprite = new Sprite(getTexture('noah-east'));
    // Anchor Y 0.55 shifts sprite down ~6px so the character's feet
    // align with the capsule's bottom contact point on the ground.
    noahSprite.anchor.set(0.5, 0.55);
    worldContainer.addChild(noahSprite);
    const sw = NOAH_SIZE * NOAH_SCALE;
    const sh = NOAH_SIZE * NOAH_SCALE;
    world.addComponent(entity, createSprite(noahSprite, sw, sh));
    // Transform scale drives the RenderSystem — must match sprite scale
    world.addComponent(entity, createTransform(x, y, 0, NOAH_SCALE, NOAH_SCALE));
  } else {
    ...
    // Transform scale drives the RenderSystem — must match sprite scale
    world.addComponent(entity, createTransform(x, y, 0, SPRITE_SCALE, SPRITE_SCALE));
